pub mod room_handler {
    ...
    impl Room for EntranceRoom {
        fn description(&self) -> String {
            String::from(
                "You wake up in a dirty unkempt room, no doors on any of the walls. \
                 A certain wall to the right of you seems brittle and ready to crumble. \
        ...
        fn handle(&self, player: &mut Player) -> Event {
            ...
            loop {
                let input = get_input();
                if !(player.visitedHash.contains(&1)) {
                    if input.contains("hit") || input.contains("break") || input.contains("destroy")
                    {
    ...
    impl Room for AxeRoom {
        fn description(&self) -> String {
            String::from(
                "You look around this room. You see a door on the left wall, \
                 as well as an axe laying on the ground in front of you. \
    ...
    impl Room for BodyRoom {
        fn description(&self) -> String {
            String::from("Walking in you get the whiff of some god awful aroma, upon inspection you realize that the room is filled with bodies, \
            ...
            if !(player.inventory.contains(&Item::Torch)) {
                ...
                loop {
                    ...
                    if input.contains("go") && input.contains("back") {
                        println!(
                            "You put the torch away and walk back to the room you came from\n"
                        );
                        return Event::MoveLeft;
                    } else if input.contains("check") && input.contains("inventory") {
                        print_inv(&mut player.inventory)
